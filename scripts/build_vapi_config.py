...
tools=[
 ...
 tool('schedule_appointment','Book a fictional demo appointment only after this call has saved a patient and the caller explicitly agrees.',{'patient_id':string,'preferred_date':{'type':'string','description':'YYYY-MM-DD, today or future'},'preferred_time':{'type':'string','description':'h:mm AM/PM'},'confirmed':{'type':'boolean','description':'True only after explicit booking approval.'}},['patient_id','preferred_date','preferred_time','confirmed']),
 {'type':'endCall'}
]
config={'name':'CareIntake AI — Riley','firstMessage':"Hi, I'm Riley, CareIntake's AI assistant. For this demo, use fictional details. What's your name and date of birth?",'model':{'provider':'groq','model':'openai/gpt-oss-120b','temperature':0.2,'maxTokens':1200,'messages':[{'role':'system','content':(ROOT/'docs/vapi-system-prompt.md').read_text()}],'tools':tools},'voice':{'provider':'vapi','voiceId':'Elliot','version':'2'},'transcriber':{'provider':'deepgram','model':'nova-3','language':'en'},'server':{'url':'https://YOUR_API_BASE_URL/webhooks/vapi','headers':{'X-Vapi-Secret':'REPLACE_WITH_VAPI_SECRET'},'timeoutSeconds':20},'serverMessages':['tool-calls','end-of-call-report'],'endCallMessage':'Thank you for calling CareIntake. Goodbye.','maxDurationSeconds':900,'startSpeakingPlan':{'waitSeconds':0.2},'stopSpeakingPlan':{'numWords':2,'voiceSeconds':0.2,'backoffSeconds':0.8},'artifactPlan':{'recordingEnabled':False}}
